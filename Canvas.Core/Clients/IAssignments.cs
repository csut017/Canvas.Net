...
public interface IAssignments
{
    ...
    Task<Submission?> RetrieveSubmission(ulong userId, ulong courseId, ulong assignmentId, SubmissionList? opts = null, CancellationToken cancellationToken = default);
    ...
    /// <param name="cancellationToken">The <see cref="CancellationToken"/> that can be used to cancel the operation.</param>
    /// <returns>A <see cref="Submission"/> instance if found, <c>null</c> otherwise.</returns>
    Task<Submission?> RetrieveSubmission(User user, Course course, Assignment assignment, SubmissionList? opts = null, CancellationToken cancellationToken = default);
