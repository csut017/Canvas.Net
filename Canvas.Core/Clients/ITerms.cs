...
public interface ITerms
{
    ...
    IAsyncEnumerable<Term> ListForAccount(ulong accountId, List? opts = null, CancellationToken cancellationToken = default);
    ...
    /// <param name="cancellationToken">The <see cref="CancellationToken"/> that can be used to cancel the operation.</param>
    /// <returns>An <see cref="IQueryable{Term}"/> containing the terms for the account.</returns>
    IAsyncEnumerable<Term> ListForAccount(Account account, List? opts = null, CancellationToken cancellationToken = default);
